  ...
  const verifyToken = async (passedToken) => {
    ...
    if (passedToken){
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_ENDPOINTS}/auth/verify-user`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ token: passedToken, recaptchaToken: recaptcha  }),
        });
        if (response.ok) {
          ...
          <LoginForm>
              <FormContainer>
                  ...
                  <LoginTitleDiv>
                        ...
                        { verifyMessage === 'Success' ? <Form378> 
                         
                        <ButtonDiv>
                        <Title color="black"> Success</Title>
                        <Button  isenable={'valid'} bgcolor={theme.colors.primary}
                        ...
                        : <Form378> <Title color="black">Something went wrong... Please try again or contact us 😊</Title> </Form378> }
